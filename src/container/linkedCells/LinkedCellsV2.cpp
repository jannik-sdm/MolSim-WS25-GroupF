...
LinkedCellsV2::LinkedCellsV2(std::vector<Particle> &particles, const Vector3 domain, const double cutoff, bool is2D,
...
void LinkedCellsV2::setNeighbourCells(const int cellIndex) {
...
std::array<int, 3> LinkedCellsV2::index1dToIndex3d(const int cellIndex) {
...
int LinkedCellsV2::index3dToIndex1d(const int x, const int y, const int z) {
  return x + numCellsX * y + numCellsX * numCellsY * z;
}

std::array<int, 3> LinkedCellsV2::coordinate3dToIndex3d(const double x, const double y, const double z) {
...
int LinkedCellsV2::coordinate3dToIndex1d(const double x, const double y, const double z) {
  std::array<int, 3> index3d = coordinate3dToIndex3d(x, y, z);
  return index3dToIndex1d(index3d[0], index3d[1], index3d[2]);
}

double LinkedCellsV2::getBorderDistance(const int cellIndex, const int border, Vector3 pos) {
...
int LinkedCellsV2::getSharedBorder(int ownIndex1d, int otherIndex1d) {
...
void LinkedCellsV2::moveParticles() {
  for (int i = 0; i < cells.size(); i++) {
...
void LinkedCellsV2::updateGhost() {
...
void LinkedCellsV2::createGhostParticles(Particle &particle, const int cell_index, Cell &cell) {
  for (int l = 0; l < 6; l++) {
...
int LinkedCellsV2::getPeriodicEquivalentForGhost(const int cellIndex, const int ghostCellIndex) {
...
const std::vector<int> LinkedCellsV2::getSharedBordersIndex(const int ownIndex1d, const int otherIndex1d) {
...
BorderType LinkedCellsV2::getSharedBorderType(const int ownIndex1d, const int otherIndex1d) {
  const auto &ownCell = cells[ownIndex1d];
  return best_of(ownCell.borders, getSharedBordersIndex(ownIndex1d, otherIndex1d));
}

double LinkedCellsV2::calcRepulsingDistance(double sigma1, double sigma2) {
