  ...
  for (auto p : parts) {
    if (p["cuboid"]) {
      ...
      ParticleGenerator::cuboid(particles, x, n, distance, mass, epsilon, sigma, v);
        ...
        std::optional<std::vector<std::array<int, 2>>> upwardsParticlesIndexes = membrane["upwards_particles_indexes"].as<std::vector<std::array<int, 2>>>();
      ...
      spdlog::debug("Generating membrane: position={} size={} distance={} mass={} velocity={} sigma={} epsilon={}",
        ...
        ParticleGenerator::membrane(particles, x, n, distance, mass, epsilon, settings.membrane.sigma, v,
          settings.membrane.upwardsParticles, upwardsParticlesIndexes.value_or(std::vector<std::array<int, 2>>{{0,0}}));
    }
    else if (p["single"]) {
