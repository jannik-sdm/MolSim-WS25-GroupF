...
class MembraneSimulation : public ThermostatSimulation {
 ...
 public:
  MembraneSimulation(std::vector<Particle> &particles, const double start_time, const double end_time,
                     ...
                     double target_temperature, double maximum_temperature_change, double r0, double stiffnessConstant,
                     double F_zUp, std::vector<Particle *> &upwardsParticles)
      ...
      : ThermostatSimulation(particles, start_time, end_time, delta_t, brown_motion_avg_velocity, dimension,
                             std::pow(2, 1.0 / 6.0) * particles[0].getSigma(), border, is2D, g_grav, t_initial, n,
                             target_temperature, maximum_temperature_change),
        r0(r0),
