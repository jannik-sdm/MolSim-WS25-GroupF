...
Particle::Particle(const Vector3 &x_arg, const Vector3 &v_arg, const double m_arg, const Vector3 &f_arg,
                   const Vector3 &old_f_arg, const int type_arg, const double epsilon, const double sigma)
    : x(x_arg), v(v_arg), f(f_arg), old_f(old_f_arg), m(m_arg), type(type_arg), epsilon(epsilon), sigma(sigma) {
  spdlog::debug("Particle generated!");
}
