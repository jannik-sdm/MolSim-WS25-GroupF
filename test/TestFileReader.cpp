...
TEST(Reader, XYZ) {
  ...
  EXPECT_DOUBLE_EQ(p[1], particles[0].getX()[1]);
  EXPECT_DOUBLE_EQ(p[2], particles[0].getX()[2]);

  EXPECT_DOUBLE_EQ(v[0], particles[0].getV()[0]);
  EXPECT_DOUBLE_EQ(v[1], particles[0].getV()[1]);
  EXPECT_DOUBLE_EQ(v[2], particles[0].getV()[2]);
